...
const Headers = () => {
  return (
    ...
    <section className='flex items-center gap-2 bg-slate-200 w-[32rem] rounded-md'><CiSearch />
    <input className='w-full text-lg bg-slate-200' type="text" placeholder="Search for Products, Brands and More" value="" ></input></section>
    <Link to={"/login"} className='relative group transition-all'>
     ...
     <div className=' flex justify-around absolute top-10 hidden w-60 flex-col gap-1 rounded-md bg-white py=3  shadow-md transition-all group-hover:flex '>
      ...
      <Link to={"/login"} className='flex gap-2 '><BsBox />Orders</Link>
